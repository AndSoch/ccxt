...
class blockbid (Exchange):
    ...
    async def fetch_balances(self, params={}):
        await self.load_markets()
    ...
    async def cancel_orders(self, side=None, params={}):
        ...
        if side is not None:
            req['side'] = side
        response = await self.privateDeleteOrders(self.extend(req, params))
        err = self.handle_error(response)
        if err:
            raise ExchangeError(self.id + ' has thrown an error: ' + err)
    ...
    async def fetch_order(self, id, symbol=None, params={}):
        ...
        await self.load_markets()
        response = await self.privateGetOrdersId(self.extend({'id': str(id)}, params))
        err = self.handle_error(response)
        if err:
            raise ExchangeError(self.id + ' has thrown an error: ' + err)
    ...
    async def fetch_withdrawals(self, code=None, since=None, limit=None, params={}):
        ...
        for i in range(0, len(self.supportedFiat)):
            if currencyCode == self.supportedFiat[i]:
                isFiat = True
        if isFiat:
            response = await self.privateGetWithdrawsFiat(self.extend(request, params))
            err = self.handle_error(response)
            if err:
                raise ExchangeError(self.id + ' has thrown an error: ' + err)
            return self.parseTransactions(response, currency)
        else:
            response = await self.privateGetWithdrawsCrypto(self.extend(request, params))
            err = self.handle_error(response)
            if err:
                raise ExchangeError(self.id + ' has thrown an error: ' + err)
            return self.parseTransactions(response, currency)
