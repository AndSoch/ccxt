...
class blockbid (Exchange):
    ...
    async def fetch_trades(self, symbol, since=None, limit=50, params={}):
        ...
        response = await self.publicGetTrades(
            self.extend({
                'market': market['id'],
                'limit': limit,
            }, params)
    ...
    async def cancel_order(self, id, params={}):
        ...
        response = await self.privateDeleteOrdersId(
            self.extend({
                'id': id,
            }, params)
        )
    ...
    async def fetch_order(self, id, symbol=None, params={}):
        ...
        await self.load_markets()
        response = await self.privateGetOrdersId(
    ...
    async def fetch_withdrawals(self, code=None, since=None, limit=None, params={}):
        ...
        if self.supportedFiat.find(currencyCode) != -1:
            ...
            response = await self.privateGetWithdrawsCrypto(
                self.extend(request, params)
            )
            err = self.handle_error(response)
            if err:
